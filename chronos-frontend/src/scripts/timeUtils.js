...
function computeCurrentTime() {
  setInterval(() => {
    ...
    spanTime.innerText = `${padNumber(hours)}:${padNumber(minutes)}:${padNumber(
      seconds
    )}`;
  }, 1000);
}
...
function getShift(timeIn) {
  ...
  if (
    parseInt(timeIn.slice(0, 2)) >= 18 &&
    parseInt(timeIn.slice(0, 2)) <= 21
  ) {
    return "C";
  }
}
...
function getElapseTime(timeEvent) {
  ...
  const diffGMT = parseInt(
    timeEventStartTimeString.match(/GMT-\d+/)[0].slice(5, 6)
  );
