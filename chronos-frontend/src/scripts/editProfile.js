function getOldEmployeeData() {
  const employee =
    ...
    JSON.parse(localStorage.getItem("data")).employee;
...
function generateEditProfileCard() {
  ...
  localStorageData.employee.position !== "Manager" &&
    listOfEmployeePositions.splice(indexOfManager, 1);

  const positionOptions = listOfEmployeePositions.map((position) => {
    const selected =
      localStorageData.employee.position === position ? "selected" : null;
  ...
  const editProfileCard = `
        ...
        <form id="editProfileForm">
            ...
            <input id="profileFirstName" class="editProfileInputs" type="text" value=${capitalize(
              localStorageData.employee.first_name
            )}></input>

            <p class="profileCaption">Last name</p>
            <input id="profileLastName" class="editProfileInputs" type="text" value=${capitalize(
              localStorageData.employee.last_name
            )}></input>
            ...
            <input id="profileEmail" class="editProfileInputs" type="text" value=${
              localStorageData.employee.email
            }></input>
...
function updateProfile() {
  ...
  const toProceedWithUpate = checkChangesBeforeFetchRequest(
    profileFirstName,
    profileLastName,
    profilePosition,
    profileEmail
...
function checkChangesBeforeFetchRequest(
  profileFirstName,
  profileLastName,
  profilePosition,
  profileEmail
  ...
  const oldEmployeeData = getOldEmployeeData();
  return (
    profileFirstName.value !== oldEmployeeData.first_name ||
    profileLastName.value !== oldEmployeeData.last_name ||
    profilePosition.value !== oldEmployeeData.position ||
    profileEmail.value !== oldEmployeeData.email
  );
}

function updateEmployeeTag(oldEmployeeData, newEmployeeData) {
  const changedAttributes = checkChangedAttributes(
    oldEmployeeData,
    newEmployeeData
  );

  if (
    changedAttributes.includes("first_name") ||
    changedAttributes.includes("last_name")
  ) {
    document.getElementById("employee-name").innerText = `${capitalize(
      newEmployeeData.first_name
    )} ${capitalize(newEmployeeData.last_name)}`;
  }

  if (changedAttributes.includes("position")) {
    document.getElementById(
      "employee-position"
    ).innerText = `${newEmployeeData.position}`;
  }
}
