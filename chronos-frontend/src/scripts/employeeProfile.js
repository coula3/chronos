  ...
  if (localStorageData) {
    const profileCard = `
            <div id="profileCard">
                ...
                <p class="profileParagraph">${capitalize(
                  localStorageData.employee.first_name
                )} ${capitalize(localStorageData.employee.last_name)}</p>

                <p class="profileCaption">Position</p>
                <p class="profileParagraph">${
                  localStorageData.employee.position
                }</p>
...
buttonProfile.addEventListener("click", (e) => {
  const isTimeEventOnEditMode = JSON.parse(
    localStorage.getItem("isTimeEventOnEditMode")
  );
  ...
  if (e.target.innerText === "Profile") {
    ...
    if (isTimeEventOnEditMode) {
      buttonClockInOut.style.cssText +=
        "margin-left: 15px; background-color: #9932CC; color: #FFF;";
    } else {
      buttonClockInOut.style.cssText +=
        "margin-left: 15px; background-color: #0000FF; color: #FFF;";
    }
    ...
    fetch(`${CHRONOS_URL}/employees/${employeeId}`, {
      ...
      .then((data) => {
        ...
        if (JSON.parse(localStorage.getItem("isTimeEventOnEditMode"))) {
          ...
          if (
            (newTimeEvent.break_start || newTimeEvent.breakStart) &&
            (newTimeEvent.break_end || newTimeEvent.breakEnd)
          ) {
...
function appendEmployeeProfile(card) {
  ...
  buttonClockInOut.style.cssText =
    "margin-left: 15px; background-color: null, color: #000;";
