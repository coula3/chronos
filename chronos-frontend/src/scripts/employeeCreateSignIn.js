...
function stageSignInButton() {
  ...
  signInEmailInput &&
    signInEmailInput.addEventListener("focus", (e) => {
  ...
  signInPasswordInput &&
    signInPasswordInput.addEventListener("focus", (e) => {
      if (document.getElementById("first-name")) {
        collapseSignUpElements();
      }
    });
...
function collapseSignUpElements() {
  if (document.getElementById("btn-create-user").innerText === "Submit") {
    document.getElementById("div-input-elements") &&
      document.getElementById("div-input-elements").remove();
    document.getElementById("btn-create-user").innerText = "Sign Up";
    document.getElementById("btn-sign-in").disabled = false;
    document.getElementById("btn-sign-in").style.cssText =
...
function signInEmployee(e) {
  ...
  const signInMsgSpan = document.getElementById("signin-msg-span");
  const bodyObject = {
    employee: { email: signInEmail, password: signInPassword },
  };
...
function persistToLocalStorageOnSignIn(data) {
  const newTimeEvent = data.employee.time_events.find(
  ...
  const timeEventInLocalStorage = JSON.parse(
    localStorage.getItem("newTimeEvent")
  );
  const onBreak =
    timeEventInLocalStorage &&
    timeEventInLocalStorage.break_start &&
    !timeEventInLocalStorage.break_end;
  onBreak && localStorage.setItem("onBreak", true);
  data.employee.time_events[0]
...
function addSignOutBtnOnReSignInEmployee() {
  ...
  document
    ...
    .insertBefore(
      divSignUpSignIn,
      document.getElementById("div-employee-tag-name")
    );
...
function reStyleSignInElements() {
  ...
  document.getElementById("form-signin").style.cssText =
    "display: flex; justify-content: flex-end; width: 100%;";
  document.getElementById("div-signin").style.cssText =
    "width: 100%; margin: 0; padding: 0; border: none;";
  buttonSignIn.innerText = "Sign Out";
  buttonSignIn.disabled = false;
  buttonSignIn.style.cssText =
    "width: 100px; background-color: #9932CC; color: #FFF; margin-right: 0;";
}
