...
function appendImageOfWorkers() {
  ...
  imageDiv.style.cssText +=
    "background: url('images/workers.png') no-repeat bottom center/cover;";
}

function instantiateEmployeeObject(employeeObject) {
  return new Employee(
    employeeObject.id,
    employeeObject.first_name,
    employeeObject.last_name,
    employeeObject.position,
    employeeObject.email,
    employeeObject.time_events
  );
}
...
function renderWelcomeMsg(employeeObject) {
  ...
  if (
    employeeObject.timeEvents &&
    employeeObject.timeEvents.some((event) => event.time_out)
  ) {
  ...
  if (
    document.getElementById("div-time-events") &&
    document.getElementById("div-time-events").innerText === ""
  ) {
    document.getElementById("div-time-events").style.backgroundColor =
      "#F5F5F5";
  }
}
...
function appendClockInOutButton() {
  ...
  buttonClockInOut.style.cssText = "background-color: #0000CD; color: #FFF;";
...
function appendProfileButton() {
  ...
  document
    .getElementById("div-menu")
    .insertBefore(buttonProfile, document.getElementById("btn-clock-in-out"));
...
function appendAdminButton() {
  ...
  document
    .getElementById("div-menu")
    .insertBefore(buttonAdmin, document.getElementById("btn-clock-in-out"));
}
...
function capitalize(string) {
  ...
  const toUpdateString = stringToArray.some(
    (string) => string[0] === string[0].toLowerCase()
  );

  if (toUpdateString) {
    const newArray = stringToArray.map(
