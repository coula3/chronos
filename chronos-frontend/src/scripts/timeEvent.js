class TimeEvent {
  ...
  updateTimeEventOnDOM() {
    ...
    tdEventBreakStart.innerText = this.breakStart
      ? getTime(this.breakStart)
      : "";
...
buttonClockInOut.addEventListener("click", (e) => {
  createTimeEvent(e);
  document.getElementById("p-new-user-msg") &&
...
function createTimeEvent(e) {
  const employeeId = document
    ...
    .getAttribute("employee-data-id");

  if (e.target.innerText === "Clock In") {
    if (
      parseInt(Date().slice(16, 18)) >= 22 ||
      parseInt(Date().slice(16, 18)) <= 4
    ) {
      renderMessage(
        "You are not authorized to clock in at this time. Please contact your supervisor for assistance"
      );
    } else {
      const bodyObject = {
        time_event: {
    ...
    const timeEventId = document
      .querySelector("#div-time-event")
      .getAttribute("event-data-id");
    let breakEnd;

    if (
      !!document.getElementById("td-event-break-start").innerText &&
    ...
    fetch(`${CHRONOS_URL}/time_events/${timeEventId}`, configObj)
      ...
      .then((data) => {
        ...
        const currentTimeEvent = new TimeEvent(
          timeEvent.id,
          timeEvent.date,
          timeEvent.time_out,
          timeEvent.break_start,
        ...
        buttonBreakResume.style.cssText =
          "width: 25px; height: 25px; font-size: 0.875rem; padding: 2px; border: transparent; border-radius: 50%; color: #FFF; background-color: #FF0000; display: inline-block;";
        ...
        buttonUpdateTimeEvents.style.cssText =
          "width: 25px; height: 25px; margin-right: 6px; font-size: 0.875rem; padding: 2px; border: transparent; border-radius: 50%; color: #FFF; background-color: #008000; display: inline-block;";
        document
          .getElementById("div-time-event-buttons")
          .insertBefore(buttonUpdateTimeEvents, buttonBreakResume);

        addNewTimeEventToTimeEvents(buttonUpdateTimeEvents);

        localStorage.setItem("runningTimeStarted", false);
        localStorage.getItem("onBreak") &&
          localStorage.setItem("onBreak", false);

        if (currentTimeEvent.timeOut) {
          (function updateHours() {
            const hrs = getHours(
              currentTimeEvent.date,
              currentTimeEvent.timeOut
            );
...
function getEmployeeLastTimeEvent(data) {
  const sortedTimeEvents = [...data.employee.time_events].sort(
    (a, b) => a.id - b.id
  );
...
function renderEmployeeTimeEvents(employeeObject) {
  ...
  const employeeTimeEvents = employeeObject.timeEvents.map(
    (event) =>
      new TimeEvent(
        event.id,
        event.date,
  ...
  const closedtimeEvents = employeeTimeEvents.filter((e) => {
    return e.timeOut;
  });
  const renderedTimeEvents = [...closedtimeEvents]
    .sort((a, b) => a.id - b.id)
    .slice(-5);
  const openTimeEvent = employeeTimeEvents.filter((e) => {
    return !e.timeOut;
  });
  ...
  if (employeeTimeEvents.filter((event) => event.timeOut).length > 0) {
    ...
    renderedTimeEvents.forEach((event) => {
      tableOfEvents += `
            <tr>
                <td class="td-events" id="td-events-col-a" style="text-align:center;">${
                  renderedTimeEvents.indexOf(event) + 1
                }</td>
                <td class="td-events" id="td-events-date">${
                  event.date ? getDate(event.date) : ""
                }</td>
                <td class="td-events" id="td-events-time-in">${
                  event.date ? getTime(event.date) : ""
                }</td>
                <td class="td-events" id="td-events-break-start">${
                  event.breakStart ? getTime(event.breakStart) : ""
                }</td>
                <td class="td-events" id="td-events-break-end">${
                  event.breakEnd ? getTime(event.breakEnd) : ""
                }</td>
                <td class="td-events" id="td-events-time-out">${
                  event.timeOut ? getTime(event.timeOut) : ""
                }</td>
                <td class="td-events" id="td-events-time-shift">${getShift(
                  getTime(event.date)
                )}</td>
                <td class="td-events" id="td-events-col-z">${getHours(
                  event.date,
                  event.timeOut
                )}</td>
            </tr>`;
    ...
    <table id="tbl-event">
        ...
        <tbody>
            <tr>
                ...
                <td class="td-event" id="td-event-date">${
                  event.date ? event.date.slice(0, 10) : ""
                }</td>
                <td class="td-event" id="td-event-time-in" >${
                  event.date ? getTime(event.date) : ""
                }</td>
                <td class="td-event" id="td-event-break-start" >${
                  event.breakStart ? getTime(event.breakStart) : ""
                }</td>
                <td class="td-event" id="td-event-break-end" >${
                  event.breakEnd ? getTime(event.breakEnd) : ""
                }</td>
                <td class="td-event" id="td-event-time-out" >${
                  event.timeOut ? getTime(event.timeOut) : ""
                }</td>
                <td class="td-event" id="td-event-time-shift" >${getShift(
                  getTime(event.date)
                )}</td>
  ...
  buttonUpdateTimeEvents.addEventListener("click", () => {
    ...
    fetch(`${CHRONOS_URL}/employees/${employeeId}`, {
      ...
      .then((data) => {
        ...
        JSON.parse(localStorage.getItem("selfTimeout")) &&
    ...
    fetch(`${CHRONOS_URL}/time_events/${timeEventId}`, {
      ...
      .then((json) => {
        if (json.message === "Delete successful") {
          ...
          if (
            JSON.parse(localStorage.getItem("data")).employee.time_events
              .length === 1
          ) {
